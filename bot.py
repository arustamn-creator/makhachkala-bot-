...
MINI_APP_URL = "https://project-d7s9l.vercel.app/index.html"
ADMIN_ID = 5715605642

# СБП — реквизиты для приёма оплаты
SBP_PHONE = "+7 9919883605"   # Номер телефона для СБП
SBP_BANK = "Тинькофф Банк"
SBP_RECIPIENT = "Абдулбеков Рустам"
# ================================
...
def cmd_start(message):
    ...
    text = (
        ...
        "Здесь ты найдёшь:\n"
        "🔧 *Мастеров* — сантехников, электриков, отделочников\n"
        "🏪 *Магазины* — стройматериалы, плитка, электрика\n"
        "🚚 *Доставку* — газель, КамАЗ, курьер по Махачкале\n"
        "💎 *Тарифы* — размести своё объявление\n\n"
    ...
    bot.send_message(
        message.chat.id, "Или выбери раздел из меню:",
...
def cmd_help(message):
    text = (
        ...
        "1️⃣ Нажми *«Открыть приложение»* — откроется полный каталог\n\n"
        "2️⃣ Выбери нужного *мастера* по категории\n\n"
        "3️⃣ Посмотри рейтинг, отзывы — нажми *«Вызвать мастера»*\n\n"
        "4️⃣ Заполни форму: имя, телефон, описание задачи\n\n"
        "5️⃣ Мастер *свяжется с тобой в течение 15 минут* ✅\n\n"
        "───────────────\n"
        "*Для мастеров:* зарегистрируйтесь и выберите тариф 💎\n"
...
def send_masters(chat_id):
    ...
    kb.add(*[InlineKeyboardButton(n, callback_data=c) for n, c in categories])
    kb.add(InlineKeyboardButton(
        "🚀 Открыть все в приложении",
        web_app=telebot.types.WebAppInfo(url=MINI_APP_URL)
    ))
...
def send_howto(chat_id):
    text = (
        ...
        "3. Выбери тариф и получай заявки\n\n"
...
def send_become_master(chat_id):
    text = (
        "🛠️ *Стать мастером*\n\n"
        "Зарегистрируйся и получай клиентов каждый день!\n\n"
        "📋 *Что нужно:*\n"
...
def callback_handler(call):
    ...
    if call.data in CATEGORY_TEXTS:
        ...
        plan_key = call.data[4:]  # "basic", "pro", "premium"
